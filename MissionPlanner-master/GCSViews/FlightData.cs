...
// square
        ...
        {
            //Used for Intializiation of GuidedMode Button Use
            //Added by Patrick
            Thread GuidedModeThread = new Thread(GuidedMode);
            GuidedModeThread.IsBackground = true;
            CalPolyPomonaFunctions.GuidedModeValue.GuidedModeClickCount++;
            button4.Text = "Guided Mode";
            if (CalPolyPomonaFunctions.GuidedModeValue.GuidedModeClickCount%2 == 0)
            {
                while (!GuidedModeThread.IsAlive)
                {
                    CalPolyPomonaFunctions.GuidedModeValue.QUIT = false;
                    GuidedModeThread.Start();
                }
            ...
            else
            {
                ...
                CalPolyPomonaFunctions.GuidedModeValue.QUIT = true;
                GuidedModeThread.Interrupt();
                GuidedModeThread.Abort();
            }
            /*
            ...
            else if ((button4.BackColor == Color.White) || (button4.BackColor == Color.Green))
            {
                CalPolyPomonaFunctions.GuidedModeValue.QUIT = true;
                button4.BackColor = Color.Red;
            }
        ...
        {
            //Usage in GuidedMode Button Click in private void button4_Click
            //Added by Patrick 

            if (CalPolyPomonaFunctions.GuidedModeValue.GuidedModeMissionInitialize == 0)
            {
                CalPolyPomonaFunctions.GuidedModeActual();
                CalPolyPomonaFunctions.GuidedModeValue.GuidedModeMissionInitialize++;
            }
            while (CalPolyPomonaFunctions.GuidedModeValue.QUIT == false)
            {
                button4.BackColor = Color.Green;
                if (CalPolyPomonaFunctions.GuidedModeValue.GuidedModeMissionInitialize == 1)
                {
                    //initializing waypoint sequence
                    CalPolyPomonaFunctions.GuidedModeValue.waypoint_next = 1;
                    CalPolyPomonaFunctions.GuidedModeValue.waypoint_inprogress = 0;
                    CalPolyPomonaFunctions.GuidedModeValue.waypoint_previous = 0;
                    CalPolyPomonaFunctions.GuidedModeValue.GuidedModeMissionInitialize++;
                    CalPolyPomonaFunctions.GuidedModeValue.waypointaccuracy_ft = 25; //will later convert degrees to ft to check accuracy
                    CalPolyPomonaFunctions.GuidedModeValue.waypoint_setdistance = 20; //latitude and longitude distance in degrees
                }

                if (CalPolyPomonaFunctions.GuidedModeValue.waypoint_inprogress < CalPolyPomonaFunctions.GuidedModeValue.waypoint_next)
                {
                    //CalPolyPomonaFunctions.GuidedModeValue.waypoint_inprogress++;
                    //Below is modified taken from flytoHereAlt
                    //------------------------------------------------------------------
                    /*
                    string alt = "100";
                    ...
                    */
                    //Below is modified taken from goHereToolStripMenu_Click
                    //------------------------------------------------------------
                    if (!MainV2.comPort.BaseStream.IsOpen)
                    {
                        CustomMessageBox.Show(Strings.PleaseConnect, Strings.ERROR);
                    ...
                    Locationwp gotohere = new Locationwp();
                    CalPolyPomonaFunctions.GuidedModeValue.specific_i = 0 + (4 * CalPolyPomonaFunctions.GuidedModeValue.waypoint_previous);

                    gotohere.id = (ushort)MAVLink.MAV_CMD.WAYPOINT;
                    gotohere.alt = (float)(Convert.ToDouble(CalPolyPomonaFunctions.GuidedModeValue.values_mission[CalPolyPomonaFunctions.GuidedModeValue.specific_i + 3]) * 0.3048); // back to m, alt must be in meters
                    gotohere.lat = Convert.ToDouble(CalPolyPomonaFunctions.GuidedModeValue.values_mission[CalPolyPomonaFunctions.GuidedModeValue.specific_i + 1]);
                    gotohere.lng = Convert.ToDouble(CalPolyPomonaFunctions.GuidedModeValue.values_mission[CalPolyPomonaFunctions.GuidedModeValue.specific_i + 2]);

                    Locationwp gotohere_guided = new Locationwp();
                    gotohere_guided.id = (ushort)MAVLink.MAV_CMD.WAYPOINT;

                    //Setting up condition
                    double x1 = MainV2.comPort.MAV.cs.lng;
                    double y1 = MainV2.comPort.MAV.cs.lat;
                    double x2 = gotohere.lng;
                    double y2 = gotohere.lat;
                    double d = CalPolyPomonaFunctions.GuidedModeValue.waypoint_setdistance;

                    if (y2 < y1)//(gotohere.lat < MainV2.comPort.MAV.cs.lat) //plane is north of target
                    {
                        if(x2 < x1) //(gotohere.lng < MainV2.comPort.MAV.cs.lng) //plane is east of target
                        {
                            gotohere_guided.alt = gotohere.alt; //NOTE: Need to separate the altitude change into a separate command
                            // Guided Mode has two different separate functions for a plane: change of altitude or new waypoint at current altitude
                            double theta = Math.Atan((x1-x2)/(y1-y2));
                            double x3 = x1 - d * Math.Sin(theta);
                            double y3 = y1 - d * Math.Cos(theta);
                            gotohere_guided.lat = y3;
                            gotohere_guided.lng = x3;

                        }
                        else //plane is west of target
                        {
                            gotohere_guided.alt = gotohere.alt; //NOTE: Need to separate the altitude change into a separate command
                            // Guided Mode has two different separate functions for a plane: change of altitude or new waypoint at current altitude
                    ...
                    else // plane is south of target
                    {
                        if(x2 < x1) //(gotohere.lng < MainV2.comPort.MAV.cs.lng) //plane is east of target
                        {
                            gotohere_guided.alt = gotohere.alt; //NOTE: Need to separate the altitude change into a separate command
                            // Guided Mode has two different separate functions for a plane: change of altitude or new waypoint at current altitude
                            double theta = Math.Atan((x1 - x2) / (y2 - y1));
                            double x3 = x1 - d * Math.Sin(theta);
                            double y3 = y1 + d * Math.Cos(theta);
                            gotohere_guided.lat = y3;
                            gotohere_guided.lng = x3;
                        ...
                        {
                            gotohere_guided.alt = gotohere.alt; //NOTE: Need to separate the altitude change into a separate command
                            // Guided Mode has two different separate functions for a plane: change of altitude or new waypoint at current altitude MAYBE???
                            double theta = Math.Atan((x2 - x1) / (y2 - y1));
                            double x3 = x1 + d * Math.Sin(theta);
                            double y3 = y1 + d * Math.Cos(theta);
                            gotohere_guided.lat = y3;
                            gotohere_guided.lng = x3;
                    ...
                    try
                    {
                        MainV2.comPort.setGuidedModeWP(gotohere_guided);
                    }
                    catch (Exception ex)
                    {
                        MainV2.comPort.giveComport = false;
                ...
                //insert degrees to ft calculatiosn here!!!! STILL NEED TO DO THAT
                //Current Position Estimation

                double current_lat = Convert.ToDouble(MainV2.comPort.MAV.cs.lat);
                double current_lng = Convert.ToDouble(MainV2.comPort.MAV.cs.lng);
                bool California = true; // California = true if we are in California, California = false if we are in Maryland
                CalPolyPomonaFunctions.LatLngConversion_toft(California, current_lat, current_lng, out double current_lat_ft, out double current_lng_ft);
                double target_waypoint_lat = Convert.ToDouble(CalPolyPomonaFunctions.GuidedModeValue.values_mission[CalPolyPomonaFunctions.GuidedModeValue.specific_i + 1]);
                double target_waypoint_lng = Convert.ToDouble(CalPolyPomonaFunctions.GuidedModeValue.values_mission[CalPolyPomonaFunctions.GuidedModeValue.specific_i + 2]);
                CalPolyPomonaFunctions.LatLngConversion_toft(California, target_waypoint_lat, target_waypoint_lng, out double target_waypoint_lat_ft, out double target_waypoint_lng_ft);
                if (((Math.Sqrt(Math.Pow(current_lat_ft, 2) + Math.Pow(current_lng_ft, 2))) - Math.Sqrt(Math.Pow(target_waypoint_lat_ft, 2) + Math.Pow(target_waypoint_lng_ft, 2))) < CalPolyPomonaFunctions.GuidedModeValue.waypointaccuracy_ft)
                {
                    CalPolyPomonaFunctions.GuidedModeValue.waypoint_previous++;
                }
            }

            while (CalPolyPomonaFunctions.GuidedModeValue.QUIT == true)
            {
                button4.BackColor = Color.Red;
            }

            //-----------------------------------------------------------------------------
            //Prior to Failed Guided Mode Test, Before Dec.14
            //-----------------------------------------------------------------------------
            /*
            if (CalPolyPomonaFunctions.GuidedModeValue.GuidedModeMissionInitialize == 0)
            {
            ...
            while(CalPolyPomonaFunctions.GuidedModeValue.QUIT == false)
            {
                ...
                if(CalPolyPomonaFunctions.GuidedModeValue.GuidedModeMissionInitialize == 1)
                {
                    ...
                    CalPolyPomonaFunctions.GuidedModeValue.waypoint_previous = 0;
                    CalPolyPomonaFunctions.GuidedModeValue.GuidedModeMissionInitialize++;
                    CalPolyPomonaFunctions.GuidedModeValue.waypointaccuracy_ft = 25;
                }
                ...
                if (CalPolyPomonaFunctions.GuidedModeValue.waypoint_inprogress < CalPolyPomonaFunctions.GuidedModeValue.waypoint_next)
                {
                    ...
                    CalPolyPomonaFunctions.GuidedModeValue.Loiter_Check = 1; //capable to check for loitering
                }
                //insert degrees to ft calculatiosn here!!!! STILL NEED TO DO THAT
                //Current Position Estimation
                ...
                if (((Math.Sqrt(Math.Pow(current_lat_ft,2)+Math.Pow(current_lng_ft, 2)))-Math.Sqrt(Math.Pow(target_waypoint_lat_ft, 2)+Math.Pow(target_waypoint_lng_ft, 2))) < CalPolyPomonaFunctions.GuidedModeValue.waypointaccuracy_ft)
                {
                    CalPolyPomonaFunctions.GuidedModeValue.waypoint_next++;
                }

                if ((MainV2.comPort.MAV.cs.mode == "Loiter") && (CalPolyPomonaFunctions.GuidedModeValue.Loiter_Check == 1))
                {
                    CalPolyPomonaFunctions.GuidedModeValue.Loiter_Check = 0;
                    CalPolyPomonaFunctions.GuidedModeValue.waypoint_next++;
                }
            }
            while(CalPolyPomonaFunctions.GuidedModeValue.QUIT == true)
        ...
        {
            //Not Used Currently
            //Added by Patrick in private void mainloop()

            if (CalPolyPomonaFunctions.InitializeButtons.DONE == 0)
            {
                button4.BackColor = Color.Red;
                CalPolyPomonaFunctions.InitializeButtons.DONE = CalPolyPomonaFunctions.InitializeButtons.DONE + 1;
            }
        }

        private void button1_Click(object sender, EventArgs e)
        {
            if(MainV2.comPort.MAV.cs.mode == "Guided") // "Guided" "Auto"
            {
                MainV2.comPort.setMode("Auto");
            }
